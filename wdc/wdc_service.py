...
logging.info('Class wdc_service X-----------------------')
...
def load_from_vcap_services(service_name):
    ...
    if vcap_services is not None:
        ...
        if service_name in services:
            logging.info('Class wdc_service - load_from_vcap_services %s' %services[service_name][0]["credentials"] )
...
class WatsonDeveloperCloudService(object):
    def __init__(self, vcap_services_name, url, username=None, password=None,
        ...
        be specified.
        vcap_services_name is personal_assistant
        """
        logging.info('Class wdc_service - WatsonDeveloperCloudService - init -----------------------')
    ...
    def request(self, method, url, accept_json=False, headers=None,
        ...
        if not data and json is not None:
            ...
            #logging.info('wdc_service headers %s ' %headers)

        auth = None
        if self.username and self.password:
            auth = (self.username, self.password)
            #logging.info('wdc_service auth %s , %s' %auth)
        if self.api_key is not None:
            #logging.info('wdc_service self.api_key is not none')
            if params is None:
                #logging.info('wdc_service params is none')
                params = {}
            if full_url.startswith(
                    'https://gateway-a.watsonplatform.net/calls'):
                params['apikey'] = self.api_key
                #logging.info('wdc_service if %s ' %params['apikey'])
            else:
                params['api_key'] = self.api_key
                #logging.info('wdc_service else %s ' %params['apikey'])

        if self.x_watson_learning_opt_out:
            headers['x-watson-learning-opt-out'] = 'true'

        #logging.info('wdc_service preparing response URL %s ' , url)
        response = requests.request(method=method, url=full_url,
                                    cookies=self.jar, auth=auth,
                                    headers=headers,
                                    params=params, data=data, files=files,
                                    **kwargs)
        #logging.info('wdc_service completed response %s', response )
